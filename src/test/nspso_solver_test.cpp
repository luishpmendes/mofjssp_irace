...
int main() {
    ...
    for (const std::string filename : {"instances/mk01.txt",
        ...
        assert(fabs(solver.omega - 0.86) < std::numeric_limits<double>::epsilon());
        assert(fabs(solver.c1 - 3.34) < std::numeric_limits<double>::epsilon());
        assert(fabs(solver.c2 - 2.30) < std::numeric_limits<double>::epsilon());
        assert(fabs(solver.chi - 0.46) < std::numeric_limits<double>::epsilon());
        assert(fabs(solver.v_coeff - 0.20) < std::numeric_limits<double>::epsilon());
        assert(solver.leader_selection_range == 28);
