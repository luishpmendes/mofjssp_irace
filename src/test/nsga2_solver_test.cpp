...
int main() {
    ...
    for (const std::string filename : {"instances/mk01.txt",
        ...
        solver = mofjssp::NSGA2_Solver(instance);
        ...
        assert(fabs(solver.crossover_probability - 0.62) <
            std::numeric_limits<double>::epsilon());
        assert(fabs(solver.crossover_distribution - 40.00) <
            std::numeric_limits<double>::epsilon());
        assert(fabs(solver.mutation_probability - 0.01) <
            std::numeric_limits<double>::epsilon());
        assert(fabs(solver.mutation_distribution - 1.18) <
            std::numeric_limits<double>::epsilon());
